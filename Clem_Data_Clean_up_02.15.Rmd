...
clemTwitterDec1 <- stl(clemTwitterzDec, s.window = "periodic")
plot(clemTwitterDec1, col="deepskyblue",lwd=3, 
     main = "Clementine: Twitter Mentions Data Decomposition")
...
# Plot seasonally adjusted time series
plot(clemTwitterSeasonallyAdjusted, xlab="Time (Year)", 
     ylab="Twitter Mentions", 
     main="Clementine: Seasonally Adjusted Twitter Mentions",
...
Indexing can be an effective means of normalizing data to a COMMON starting point (i.e All values / observations are exactly the same for the series) and observing how variables change over time relative to each other.
...
-	Since the data the original social media mentions data and sales datas are at weekly level, Indexed PD data was rolled up to weekly level and an average of that week was calculated and used as an observation. 
...
-	Calculate 7 Day Moving Average of final percent values of indexed sales volume and seasonally adjusted indexed normalized social media mentions data to account for 
bad / missing data and plot the trend lines.

- Something to consider: feature scaling techniques so that social media mentions for each 
of the series are within a particular range. 

Next steps: 

Read final cleaned up dataset of sales volume + social media mentions data and build 
a linear regression model in time series context (?) to derive the coefficients. 
